...
const ExpertJoinedSessions = () => {
  ...
  useEffect(() => {
    ...
    setActiveTab(t);
    if (t === 'completed') setCompletedSubTab(searchParams.get('sub') || 'joined');
  }, [searchParams]);
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      if (activeTab === 'announcements') {
        ...
        const res = await API.get('/sessions?status=completed&participant=' + user._id);
        const conducted = (res.data.sessions || []).filter(s => s.expert?._id === user._id || s.expert === user._id);
        const joined = (res.data.sessions || []).filter(s => s.expert?._id !== user._id && s.expert !== user._id);
        setSessions(prev => ({
          ...prev,
        ...
        setSessions(prev => ({ ...prev, myRequests: res.data.sessions || [] }));
  ...
  const getList = () => {
    if (activeTab === 'completed') {
      return completedSubTab === 'conducted' ? sessions.completedConducted : sessions.completedJoined;
    }
  ...
  const SessionCard = ({ session, showJoinButtons = false }) => (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-sm text-gray-600 mb-3">
        ...
        {session.participants?.length || 0} / {session.requiredStudents || session.maxParticipants || 25} students
      </div>
      {session.date && (
      ...
      {showJoinButtons && (session.status === 'requested' || session.status === 'pending') && (
        <div className="pt-4 border-t border-gray-200 flex gap-2" onClick={(e) => e.stopPropagation()}>
          {!isUserRequested(session) && (
            <>
              <Button size="sm" onClick={() => handleJoinClick(session, 'student')}>
                Join as Student
              </Button>
              <Button size="sm" variant="outline" onClick={() => handleJoinClick(session, 'expert')}>
                Join as Conductor
              </Button>
            </>
          )}
  ...
  const tabs = [
    ...
    { id: 'my-requests', label: 'Request History', icon: History },
  ...
  return (
    <DashboardLayout>
      <div className="p-8">
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-gray-600 mt-1">Browse announcements, join as student or conductor, and view your session history</p>
          </div>
          <Button icon={Send} onClick={openRequestSessionModal}>Request Session</Button>
        </div>
        ...
        {loading ? (
          ...
          <Card>
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">
                ...
                {activeTab === 'my-requests' && 'You have not requested any sessions yet'}
                {activeTab === 'ai-recommendations' && 'No AI recommendations available'}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {getFilteredSessions().map((s) => (
              <SessionCard key={s._id} session={s} showJoinButtons={activeTab === 'announcements' || activeTab === 'pending' || activeTab === 'ai-recommendations'} />
            ))}
          </div>
        )}
      </div>
      ...
      <Modal
        ...
        onClose={() => { setShowJoinModal(false); setSelectedSession(null); }}
        title={joinRole === 'expert' ? 'Volunteer as Conductor' : 'Request to Join as Student'}
        footer={
          <>
        ...
        {selectedSession && (
          <div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4">
              ...
              <p className="text-sm text-gray-600">{selectedSession.moduleCode}</p>
            </div>
      ...
      <Modal
        ...
        onClose={() => { setShowRequestSessionModal(false); setRequestSessionForm({ topic: '', moduleCode: '', reason: '' }); }}
        title="Request a New Session"
        footer={
          <>
            <Button variant="outline" onClick={() => setShowRequestSessionModal(false)}>Cancel</Button>
            <Button onClick={submitRequestSession} loading={requestSessionLoading} icon={Send}>Send to Admin</Button>
          </>
        }
      >
        <div>
          <p className="text-gray-600 mb-4">Submit a request for a new session. The admin will review your request.</p>
